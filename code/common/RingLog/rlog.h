...
namespace RingLog
{
	...
	public:
		...
		void init_path(const char* log_dir, const char* prog_name, int level, bool need_print);
...
#define LOG_INIT(log_dir, prog_name, level, need_print) \
    do \
    { \
        ring_log::ins()->init_path(log_dir, prog_name, level, need_print); \
        pthread_t tid; \
