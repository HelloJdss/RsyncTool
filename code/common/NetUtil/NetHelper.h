...
{
public:
    SocketAddress(){   bzero(GetAsSockAddrIn(), sizeof(*GetAsSockAddrIn()));};
...
public:
    ...
    static void FillVectorFromSet(vector<TCPSocketPtr>* outSockets, const vector<TCPSocketPtr>* inSockets, const fd_set& inSet); //inSockets (which is in inSet) ==> outSockets
