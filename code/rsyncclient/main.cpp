...
int main(int argc, char *argv[])
{
    ...
    Generator generator;
    generator.GenerateBlockInfos("./test.txt");

    auto infos = generator.GetBlockInfos("./test.txt");
    ...
    {
        blockVec.push_back(
                Protocol::CreateBlockInfo(builder, builder.CreateString(it.filename), it.splitsize, it.order, it.offset,
                                          it.length, it.checksum, builder.CreateString(it.md5)));
    }

    auto fbb = Protocol::CreateFileBlockInfos(builder, builder.CreateString(FileHelper::GetPath("./test.txt")),
                                              builder.CreateString("./Test.txt"), builder.CreateVector(blockVec));
    builder.Finish(fbb);
